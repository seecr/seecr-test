...
class PortNumberGenerator(object):
    ...
    def next(cls, blockSize=1):
        for i in xrange(cls._maxTries):
            ...
            togo = int(blockSize)
            while togo > 0:
                if portNumberToBind > 0 and portNumberToBind in cls._usedPorts:
                    break
                sok = socket()
                sok.setsockopt(SOL_SOCKET, SO_LINGER, pack('ii', 0, 0))
                sok.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
                try:
                    sok.bind(('127.0.0.1', portNumberToBind))
                except IOError, e:
                    print e
                ...
                sok.close()
                if (aPort in cls._usedPorts) or (aPort in claimedPorts):
                    break
                claimedPorts.add(aPort)
                if port is None:
                    portNumberToBind = port = aPort
                portNumberToBind += 1
                togo -= 1
            if len(claimedPorts) != blockSize:
                continue
            cls._usedPorts.update(claimedPorts)
            return port
